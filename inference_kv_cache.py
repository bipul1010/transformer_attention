...
def greedy_output(encoder_input, model, src_mask, tokenizer_tgt, max_length, device):
    ...
    kv_cache = decoder_input.clone()  ## (1,seq_len)

    while True:

        if kv_cache.shape[1] == max_length:  ## (seq_len)
            break
        ...
        kv_cache = torch.cat(
            [kv_cache, torch.tensor([next_token_id]).unsqueeze(0).to(device)],
            dim=1,
        )  ## (1,seq_len)
        decoder_input = torch.tensor([next_token_id]).unsqueeze(0).to(device)  ##(1,1)
    return kv_cache
